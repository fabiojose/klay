...
if [ "$DETACH" == "n" ]; then
  ...
  # KLAY ID     LINUX PID    TYPE   VERSION    CREATED    STATUS    PORTS   ARGS
  echo "$EXTERNAL_ID;cat '$PROCESS_DIR/pid' 2>/dev/null;cat '$PROCESS_DIR/type';cat '$PROCESS_DIR/version';$(date +"%Y-%m-%d_%H:%M:%S");bash $SCRIPTS_HOME/psw.sh $LINUX_PID;cat '$PROCESS_DIR/ports';'$@'" >> $KLAY_PROCESSES
